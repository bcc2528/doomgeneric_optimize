...
int frequencies[] = {
	   0,  175,  180,  185,  190,  196,  202,  208,
	 214,  220,  226,  233,  240,  247,  254,  262,
	 269,  277,  285,  294,  302,  311,  320,  330,
	 339,  349,  359,  370,  381,  392,  403,  415,
	 427,  440,  453,  466,  480,  494,  508,  523,
	 539,  554,  571,  587,  604,  622,  640,  659,
	 679,  698,  719,  740,  762,  784,  807,  831,
	 855,  880,  906,  932,  960,  988, 1017, 1046,
	...
	2640, 2794, 2875, 2960, 3051, 3139, 3233, 3323,
	3428, 3519, 3626, 3740, 3848, 3950, 4072, 4186,
	4323, 4435, 4571, 4716, 4850, 4992, 5143, 5279,
	5448, 5601, 5764, 5936, 6118, 6279, 6484, 6666,
};
...
static void beep_task(W arg)
{
	...
	while(task_loop)
	{
		if (current_sound_lump != NULL && current_sound_remaining > 0)
		{
			...
			if (tone < arrlen(frequencies) && frequencies[tone] != 0)
			{
				timefreq = 0x70000 + frequencies[tone];
			}
			else
			{
				timefreq = -1;
